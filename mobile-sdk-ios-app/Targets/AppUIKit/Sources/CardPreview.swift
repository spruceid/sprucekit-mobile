    ...
    init() {
        do {
            _ = try credentialPack.addJsonVc(
                jsonVc: JsonVc.newFromJson(utf8JsonString: jsonstring))
        } catch {
...
struct CardPreview: PreviewProvider {
    ...
    static var listRendering = CardRendering.list(
    ...
    static var detailsRendering = CardRendering.details(
            ...
            fields: [
                ...
                CardRenderingDetailsField(
                    ...
                    formatter: { (values: [String: [String: GenericJSON]]) in
                        ...
                        return Text(
                            "\(w3cvc["issuanceDate"]?.toString() ?? "") - \(w3cvc["expirationDate"]?.toString() ?? "")"
                        )
                    }
                ),
            ]
    ...
    static var previews: some View {
        VStack {
            Card(
                credentialPack: previewData.credentialPack,
                rendering: listRendering
            )
            .padding(12)
            Card(
                credentialPack: previewData.credentialPack,
                rendering: detailsRendering)
        }
