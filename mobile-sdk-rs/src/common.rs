use std::{cmp::Ordering, collections::HashMap, hash::Hash, ops::Deref, sync::Arc};
...
impl std::fmt::Display for CborValue {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            CborValue::Bytes(items) => {
                write!(f, "{}", items.iter().map(|i| i.to_string()).join(","))
            }
            CborValue::Text(v) => write!(f, "{}", v),
            CborValue::Array(cbor_values) => {
                write!(f, "{}", cbor_values.iter().map(|i| i.to_string()).join(","))
            }
            CborValue::ItemMap(hash_map) => write!(
                f,
                "{}",
