...
public protocol MdocProtocol: AnyObject, Sendable {
     ...
     */
    func willExpireInNumDays(numDays: Int64)  -> Bool
...
open func invalidationDate()throws  -> String  {
     ...
     * that has its own expiration date.
     */
open func isMdocExpired() -> Bool  {
    return try!  FfiConverterBool.lift(try! rustCall() {
    uniffi_mobile_sdk_rs_fn_method_mdoc_is_mdoc_expired(
            self.uniffiCloneHandle(),$0
    )
})
}
...
open func keyAlias() -> KeyAlias  {
     ...
     * a parameterized given number of days.
     */
open func willExpireInNumDays(numDays: Int64) -> Bool  {
    return try!  FfiConverterBool.lift(try! rustCall() {
    uniffi_mobile_sdk_rs_fn_method_mdoc_will_expire_in_num_days(
            self.uniffiCloneHandle(),
        FfiConverterInt64.lower(numDays),$0
    )
})
}
...
open func inputDescriptorId() -> String  {
    return try!  FfiConverterString.lift(try! rustCall() {
    uniffi_mobile_sdk_rs_fn_method_requestedfield_input_descriptor_id(
            self.uniffiCloneHandle(),$0
    )
})
}
...
private let initializationResult: InitializationResult = {
    ...
    if (uniffi_mobile_sdk_rs_checksum_method_mdoc_is_mdoc_expired() != 50579) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_mobile_sdk_rs_checksum_method_mdoc_key_alias() != 39341) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_mobile_sdk_rs_checksum_method_mdoc_will_expire_in_num_days() != 3193) {
        return InitializationResult.apiChecksumMismatch
    }
