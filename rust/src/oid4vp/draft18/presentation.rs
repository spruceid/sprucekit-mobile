    ...
    for Draft18PresentationOptions<'_>
{
    ...
    async fn sign(
        ...
        let signature_bytes = self
            .signer
            .sign(self.active_key_id.clone(), message.to_vec())
            .await
    ...
    M: ssi::verification_methods::VerificationMethod,
{
    ...
    async fn for_method(
        ...
        Ok(method
            .controller()
            .filter(|ctrl| **ctrl == self.signer.did(self.active_key_id.clone()))
            .map(|_| self.clone()))
    }
}
...
impl Draft18PresentationOptions<'_> {
    pub async fn verification_method_id(&self) -> Result<IriBuf, Draft18PresentationError> {
        self.signer
            .verification_method(self.active_key_id.clone())
            .await
            .parse()
    ...
    pub fn issuer(&self) -> String {
        self.signer.did(self.active_key_id.clone())
    }
    ...
    pub fn jwk(&self) -> Result<JWK, Draft18PresentationError> {
        JWK::from_str(&self.signer.jwk(self.active_key_id.clone()))
