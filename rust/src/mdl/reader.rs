...
/// One document type's worth of requested elements, as `isomdl`'s non-empty namespace map.
fn build_namespaces(
    namespaces: HashMap<String, HashMap<String, bool>>,
) -> Result<device_request::Namespaces, MDLReaderSessionError> {
    let namespaces: Result<BTreeMap<_, NonEmptyMap<_, _>>, non_empty_map::Error> = namespaces
        .into_iter()
        .map(|(namespace, elements)| {
            let elements: BTreeMap<_, _> = elements.into_iter().collect();
            elements.try_into().map(|elements| (namespace, elements))
        })
        .collect();
...
pub fn establish_session(
    ...
    requested_items: HashMap<String, HashMap<String, HashMap<String, bool>>>,
    ...
    // Ordered by doctype so the request bytes do not depend on `HashMap` iteration order.
    // `isomdl` correlates the response by doctype membership rather than by position, so the
    // order is ours to pick.
    let items_requests = requested_items
        .into_iter()
        .collect::<BTreeMap<_, _>>()
        .into_iter()
        .map(|(doc_type, namespaces)| {
            Ok(ItemsRequest::new(doc_type, build_namespaces(namespaces)?))
        })
        .collect::<Result<Vec<_>, MDLReaderSessionError>>()?;
    let items_requests =
        NonEmptyVec::try_from(items_requests).map_err(|_| MDLReaderSessionError::Generic {
            value: "at least one document type must be requested".to_string(),
        })?;
...
pub struct VerifiedDocument {
    ...
    pub namespaces: HashMap<String, HashMap<String, MDocItem>>,
}
...
pub struct MDLReaderResponseData {
    ...
    pub failed_doc_types: Vec<String>,
...
/// The verified documents as JSON: an array of `{"docType", "namespaces"}` objects.
///
/// An array rather than an object keyed by doc type, because two documents in one response may
/// share a doc type and keying on it would drop one of them.
fn verified_documents_to_json(
                    ...
                    "namespaces": document
                        ...
                        .map(|(namespace, elements)| {
                            (
                                ...
                                elements
                                    .iter()
                                    .map(|(identifier, value)| (identifier.clone(), value.into()))
...
impl MDLReaderResponseData {
    pub fn verified_response_as_json(
        ...
        verified_documents_to_json(&self.documents)
    }
}
...
/// The verified documents, the failed doctypes to display, and any errors, from a validation
/// outcome.
///
/// Only documents that passed every check contribute elements: showing a holder's claimed values as
/// if they were verified would be worse than showing nothing. Verified doctypes come from the
/// signature-verified MSO and stay attached to the document they were signed with; the claimed
/// labels of documents that failed are returned separately, so the UI can name what it could not
/// verify without those labels ever being mistaken for authenticated ones.
...
/// The three fields both response records carry, derived once from a validation outcome.
struct DerivedResponse {
    documents: Vec<VerifiedDocument>,
    failed_doc_types: Vec<String>,
    errors: Option<String>,
}

fn derive_response(
    outcome: &ResponseValidationOutcome,
) -> Result<DerivedResponse, MDLReaderResponseError> {
    let errors =
...
mod tests {
    use isomdl::{
        ...
        presentation::authentication::{
            DocumentError, FailedDocument, RejectedDocument, ResponseError, ValidatedDocument,
            VerifiedMso,
        },
    };
    ...
    /// A document that passed every check, disclosing one element in one namespace.
    fn validated(
        doc_type: &str,
        namespace: &str,
        identifier: &str,
        ...
        namespaces.insert(
            namespace.to_string(),
            [(identifier.to_string(), serde_json::json!(value))]
                .into_iter()
                .collect(),
        );
        ValidatedDocument {
            ...
            mso: VerifiedMso {
                ...
                signed: OffsetDateTime::UNIX_EPOCH,
    ...
    /// The reason the response is reported per document rather than as one merged namespace map:
    /// two credentials in one response may share a namespace, and merging them let whichever came
    /// second silently overwrite the first's elements. Both must survive, each attached to the
    /// doctype its own MSO signed.
    #[test]
    fn two_documents_sharing_a_namespace_keep_their_own_elements() {
        let mut outcome = outcome();
        outcome.documents = vec![
            validated(
        ...
        let DerivedResponse {
            ...
            failed_doc_types,
            errors,
        } = derive_response(&outcome).expect("derivation failed");

        assert!(errors.is_none());
        assert!(failed_doc_types.is_empty());
        assert_eq!(documents.len(), 2, "neither document may be dropped");
        for (document, expected_doc_type, expected_country) in [
            (&documents[0], "org.iso.18013.5.1.mDL", "US"),
            (&documents[1], "org.iso.23220.photoid.1", "UT"),
        ] {
            assert_eq!(document.doc_type, expected_doc_type);
            let element = &document.namespaces["org.iso.23220.1"]["issuing_country"];
            assert!(
                matches!(element, MDocItem::Text(country) if country == expected_country),
                "{expected_doc_type} lost its own element: {element:?}"
            );
        }
    }
    ...
    fn a_failed_document_contributes_no_elements_but_always_reports_a_reason() {
        ...
        assert_eq!(failed_doc_types, vec!["org.iso.18013.5.1.mDL".to_string()]);
    ...
    /// verified documents or the failed-doctype list -- it was not evaluated, so it did not fail.
    /// It must still be reported, or a response that arrived and was dropped looks identical to no
    /// response at all.
    #[test]
    fn rejected_documents_contribute_no_elements_but_are_still_reported() {
        ...
        let DerivedResponse {
            ...
            failed_doc_types,
            errors,
        } = derive_response(&rejected_only).expect("derivation failed");
    ...
    fn a_clean_response_reports_no_errors() {
        let derived = derive_response(&outcome()).expect("derivation failed");
        assert!(derived.errors.is_none());
    }

    /// A request must name at least one document type. Building the request from an empty map
    /// would put an empty `DocRequests` on the wire, which the holder answers with nothing --
    /// indistinguishable from a refusal to disclose.
    #[test]
    fn a_request_naming_no_document_type_is_an_error() {
        let handover = Arc::new(ReaderHandover::new_qr("mdoc:".to_string()));
        let Err(MDLReaderSessionError::Generic { value }) =
            establish_session(handover, HashMap::new(), None)
        else {
            panic!("a request naming no document type must be refused");
        };
        assert!(
            value.contains("at least one document type"),
            "refused for the wrong reason: {value}"
        );
    }
