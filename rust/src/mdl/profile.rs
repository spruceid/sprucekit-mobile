...
pub enum CertificateProfileError {
    // Named `detail` rather than `cause`: UniFFI renders an error variant's fields as Kotlin
    // properties on a `Throwable` subclass, and `cause` collides with `Throwable.cause`.
    #[error("`{value}` is not a valid object identifier: {detail}")]
    InvalidOid { value: String, detail: String },
}
