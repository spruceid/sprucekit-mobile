    ...
    return when (type) {
      129.toByte() -> {
        return (readValue(buffer) as Long?)?.let {
          Oid4vpCompatibilityMode.ofRaw(it.toInt())
        }
      }
      130.toByte() -> {
...
interface Oid4vp {
   ...
   */
  fun handleAuthorizationRequest(url: String, mode: Oid4vpCompatibilityMode, callback: (Result<HandleAuthRequestResult>) -> Unit)
  /**
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            val modeArg = args[1] as Oid4vpCompatibilityMode
            api.handleAuthorizationRequest(urlArg, modeArg) { result: Result<HandleAuthRequestResult> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(Oid4vpPigeonUtils.wrapError(error))
              } else {
