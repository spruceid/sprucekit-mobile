...
enum PdfSupplementType: Int {
  case barcode = 0
  case opticalBarcodeCredential = 1
}
...
struct PdfSupplement: Hashable {
  ...
  static func fromList(_ pigeonVar_list: [Any?]) -> PdfSupplement? {
    ...
    let jsonldVcb: String? = nilOrValue(pigeonVar_list[3])
  ...
  static func == (lhs: PdfSupplement, rhs: PdfSupplement) -> Bool {
    ...
    return deepEqualsSpruceUtils(lhs.type, rhs.type) && deepEqualsSpruceUtils(lhs.data, rhs.data) && deepEqualsSpruceUtils(lhs.barcodeType, rhs.barcodeType) && deepEqualsSpruceUtils(lhs.jsonldVcb, rhs.jsonldVcb)
  }
  ...
  func hash(into hasher: inout Hasher) {
    ...
    deepHashSpruceUtils(value: jsonldVcb, hasher: &hasher)
  }
}
...
protocol SpruceUtils {
  ...
  func generateTestOpticalBarcodeCredential(completion: @escaping (Result<String, Error>) -> Void)
}
...
class SpruceUtilsSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: SpruceUtils?, messageChannelSuffix: String = "") {
    ...
    ///   [PdfSupplement] with `type == PdfSupplementType.opticalBarcodeCredential`
    ///   via the `jsonldVcb` field.
    let generateTestOpticalBarcodeCredentialChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.sprucekit_mobile.SpruceUtils.generateTestOpticalBarcodeCredential\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      generateTestOpticalBarcodeCredentialChannel.setMessageHandler { _, reply in
        api.generateTestOpticalBarcodeCredential { result in
          switch result {
          case .success(let res):
            reply(wrapResult(res))
          case .failure(let error):
      ...
      generateTestOpticalBarcodeCredentialChannel.setMessageHandler(nil)
    }
  }
}
